...
app.controller "editOrder", (
  ...
  getInitOrder = ()->
    Order.get($stateParams.id).then ((result) ->
      $scope.currentOrder = result
      $scope.currentCustomer= result.customer
      $scope.currentItem = result.item
      $scope.fetchAddress = result.fetchAddress
      $scope.deliveryAddress = result.deliveryAddress
      $scope.friend = result.friend

      $scope.fetchMannerChange(1)

      if $scope.currentOrder.urgent
        $scope.currentOrder.urgent = 'true'
      else
        $scope.currentOrder.urgent = 'false'
